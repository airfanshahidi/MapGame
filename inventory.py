...
items = {
      "Chest": {
            ...
            "Status": "closed",
            "Location": [2, 1],  # Row and column of the chest's location
            "Action": ["open", "inspect", "done"],
            "Requirement": ["key", None, None]  # Requirement to open the chest is a key
      },
      "Key": {
            "Description": "You find a key hanging on the wall.",
            "Status": "lost",
            "Location": [0, 1],  # Initial row and column of the key's location
            "Action": ["take", "leave"],
      ...
      "Treasure": {
            ...
            "Status": "in chest",
            "Location": [None, None],  # Location will be updated when the chest is opened
            "Action": ["take", "leave"],
            "Requirement": [None, None]
      }
}
...
def HideKey():
      ...
      for i in range(0, (int(map.max_row) + 1), 1):
            row_list.append(i)
      items["Key"]["Location"][0] = random.choice(row_list)  # Assign a random row for the key
      col_list = []
      for j in range(0, (int(map.max_col) + 1), 1):
            col_list.append(j)
      items["Key"]["Location"][1] = random.choice(col_list)  # Assign a random column for the key
...
def InspectRoom():
      """
      When the player chooses 'look' in the main menu, it triggers this
      inspect room function. This function will look through the
      game's objects and see if any are located on the player's current
      tile. If one is, then a sub-menu will populate from that object's
      dictionary.
      """
      global inventory, items
      found_object = False
      room_inventory = []
      location_description = map_layout[character.row][character.col]
...
def TreasureActions():
      ...
      This function will allow the player to take the treasure and win the
      game. The player cannot get out of this loop without ending the game.
      """
      ...
      while True:
            ...
            gold_choice = input("   Action: ").lower()
            ...
            if gold_choice == items["Treasure"]["Action"][0]:
                  ...
                  for object in items:
                        print(f"  - {object}'s Status: {items[object]['Status']}")
...
def ChestActions():
      """
      When the player chooses to inspect the chest object, this function
      is triggered so that it can provide valid options pertaining to this
      object. The main game play goal is to find a key and then open this chest.
      """
      global items, inventory
      deciding = True
      while deciding:
...
def KeyActions():
             ...
             while deciding: 
                           ...
                           if key_choice == items["Key"]["Action"][0]:  
                                        ...
                                        print("Congrats the key is now in your inventory!")
